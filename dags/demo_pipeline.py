...
with DAG(
    ...
    # ---- Task 5: PySpark 清洗转换 (在 Ubuntu VM 内执行) ----
    spark_process = BashOperator(
        ...
        bash_command=(
            ...
            f"--input data/input.jsonl "
            f"--output data/output_parquet' "
            f"|| echo '⚠️  SSH 连接失败。如果 VM 不可用，请在本地执行 PySpark 脚本。'"
        ),
        env={
            "PYTHONUNBUFFERED": "1",
        },
